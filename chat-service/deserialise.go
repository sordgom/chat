...
func DeserialiseChat(docs []Document) []Chat {
	chats := []Chat{}
...
func DeserialiseContactList(contacts []redis.Z) []ContactList {
	contactList := make([]ContactList, 0, len(contacts))

	// improvement tip: use switch to get type of contact.Member
	// handle unknown type accordingly
	for _, contact := range contacts {
		contactList = append(contactList, ContactList{
